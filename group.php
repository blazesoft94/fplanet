<?php
session_start();
include "profanity/LeoProfanity.php";
...
if(isset($_POST["post_button"])){
	...
	// $pic_loc = $_FILES['post_image']['tmp_name'];
	// $folder="img/";

	
	$filter = new LeoProfanity();
	
	$post_text = $filter->clean($_POST["post_text"]);

	// $post_text = $_POST["post_text"];
	$curr_date = date("Y-m-d");
	$curr_time = date("h:i:s");
	// move_uploaded_file($pic_loc,$folder.$pic);
	$sql = "INSERT INTO `posts` (`post_id`, `post_user_id`, `post_group_id`, `post_text`, `post_date`, `post_image`, `post_time`) VALUES (NULL, '$id', '$group_id', '$post_text', '$curr_date', NULL, '$curr_time');";
...
if(!empty($group_id)){
	$sql = "SELECT * from groups where group_auto_added = '1' and group_id = '$group_id'";
	$sql2 = "SELECT * from registrations, groups where registration_user_id = '$id' and registration_group_id = '$group_id'";
	$result_1 = $con->query($sql);
	$result_2 = $con->query($sql2);
	$found = false;
...
//	m.message_id, m.message_sender_id, u.user_name, m.message_time, m.message_date, u.user_image, m.message_content, u.user_image, m.message_status
    ...
    WHERE p.post_user_id = u.user_id and p.post_group_id = $group_id order by post_id desc ";
    $result = $con->query($sql);
    $c = 0;
	if($result->num_rows>0){
		while($row = $result->fetch_assoc()){
...
<div class="container">
	<div class="row">
										...
										<div class="tab-pane active show" id="home-1" role="tabpanel" aria-expanded="true">
											...
											<form  method="post" action="<?php echo $_SERVER["PHP_SELF"]."?group_id=".$group_id?>" >
												...
												<div class="form-group with-icon label-floating is-empty">
													...
													<textarea id="post-text" name="post_text" class="form-control" placeholder=""></textarea>
												<span class="material-input"></span></div>
												<div class="add-options-message">
													<a href="#" class="options-message" data-toggle="tooltip" data-placement="top" data-original-title="ADD PHOTOS">
													<!-- <input  name="post_image" accept="image/x-png,image/gif,image/jpeg" type="file"> -->
													...
													<button type="submit" id="post-button" name="post_button" class="btn btn-primary btn-md-2">Post Status</button>
												</div>
				...
				<div class="ui-block-content">
					<ul class="widget w-personal-info item-block">
						<li>
							...
							<span class="text"><?php echo $group_about ?></span>
						</li>
		...
		<div class="col-xl-3 order-xl-3 col-lg-6 order-lg-3 col-md-6 col-sm-12 col-xs-12">
			...
			<?php 
				$sql = "Select * from polls where poll_group_id = '$group_id'";
				$result = $con->query($sql);
				if($result->num_rows>0){
					$row = $result->fetch_assoc();
